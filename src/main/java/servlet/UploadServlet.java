        ...
        try {
            ...
            if (formItems != null && formItems.size() > 0) {
                ...
                for (FileItem item : formItems) {
                    // 处理不在表单中的字段
                    if (!item.isFormField()) {
//                        String oriFileName = new File(item.getName()).getName();
...
//                        String filePath = uploadPath + File.separator + fileName + fileType;
                        //String fileName = request.getSession().getAttribute("student.getStuno()").toString();
                        Student student = (Student) request.getSession().getAttribute("student");
                        String fileName = student.getStuno();
